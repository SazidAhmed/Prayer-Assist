...
function changeCount(prayerId: string, phaseIdx: number, delta: number) {
  ...
  const updated = def.phases.map((ph, i) =>
    i === phaseIdx
      ? { ...ph, total: Math.min(12, Math.max(1, ph.total + delta)) }
      : ph,
  );
  store.updatePrayerPhases(prayerId, updated);
}
...
function deletePhase(prayerId: string, phaseIdx: number) {
  ...
  store.updatePrayerPhases(
    prayerId,
    def.phases.filter((_, i) => i !== phaseIdx),
  );
}
...
const totalPages = computed(() =>
  Math.ceil(store.customPrayerDefs.length / pageSize),
);
const pagedPrayers = computed(() => {
  const start = (currentPage.value - 1) * pageSize;
  return store.customPrayerDefs.slice(start, start + pageSize);
});
...
// Reset page if total pages decreases
...
<template>
  <!-- Backdrop -->
  <div class="h-full w-full flex flex-col" style="background: #080c14">
    <!-- Header -->
    <div
      class="flex items-center justify-between px-5 pt-14 pb-5 border-b border-white/10"
    >
      <div>
        <p class="text-white/40 text-xs uppercase tracking-[0.25em] mb-0.5">
          Customize
        </p>
    ...
    <!-- Prayer list -->
    <div
      class="flex-1 overflow-y-auto custom-scrollbar px-4 py-4 pb-32 flex flex-col gap-3"
    >
      <!-- Add new prayer form (Moved to top) -->
      <div
        class="mb-4 rounded-3xl border border-white/10 bg-white/[0.03] overflow-hidden flex-shrink-0"
      >
      ...
      <div v-if="store.customPrayerDefs.length === 0" class="text-center py-8">
        ...
        <p class="text-white/30 text-xs mb-6">
          Add your first prayer or restore defaults.
        </p>
      ...
      <div
        v-for="def in pagedPrayers"
        :key="def.id"
        class="rounded-3xl border border-white/10 overflow-hidden flex-shrink-0 transition-all duration-300"
      ...
      >
        ...
        >
          ...
          <div class="flex-1 text-left">
            ...
            <p class="text-white/30 text-xs mt-0.5">
              {{ def.phases.length }} phases · {{ totalRakats(def.id) }} rakats
              total
            </p>
          </div>
          <span
        ...
        <Transition name="expand">
          <div v-if="expandedId === def.id" class="px-4 pb-4">
            ...
            <!-- Phase rows -->
            <div
              class="flex flex-col gap-2 mb-3 max-h-[280px] overflow-y-auto custom-scrollbar pr-1"
            >
              <div
              ...
              >
                ...
                <button
                  ...
                  :class="
                    PHASE_META[phase.type].bg +
                    ' ' +
                    PHASE_META[phase.type].color
                  "
                ...
                <!-- Count control -->
                <div
                  class="flex items-center gap-1 bg-white/5 border border-white/10 rounded-xl overflow-hidden"
                >
                  <button
                  ...
                  <span
                    class="w-7 text-center text-white font-bold tabular-nums text-sm"
                  >
                    {{ phase.total }}
                  </span>
                  <button
      ...
      <div
        v-if="totalPages > 1"
        class="mt-2 flex items-center justify-between px-2 flex-shrink-0"
      >
        <button
          class="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white/60 hover:text-white disabled:opacity-20 disabled:pointer-events-none transition-all active:scale-95 text-sm font-medium"
        ...
        <div class="flex items-center gap-1.5">
          <span
            class="text-white/40 text-xs uppercase tracking-widest font-bold"
            >Page</span
          >
          <span class="text-white font-black text-sm">{{ currentPage }}</span>
          <span class="text-white/20 text-xs">/</span>
          <span class="text-white/40 text-xs">{{ totalPages }}</span>
        </div>
